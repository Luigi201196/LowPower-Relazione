...
\end{itemize}
Di seguito sono riportate le parti più significative del file di testbench, disponibile integralmente in appendice fig. \ref{counter_tb1} \ref{counter_tb2} \ref{counter_tb3}.\\
\\
\textit{.......\\
.......\\
...
Il testo è disponibile in appendice fig \ref alla sezione “run modelsim simulation” del file \textit{testing\_script.py}.
\\
...
\hspace*{1cm}if str(InputsDecimal[i])!=str(Py\_results[i]):\\
\hspace*{2cm}\#error message for step “i”    \\
\hspace*{2cm}print("found an error in step "+str(i)+"\\n")\\
\hspace*{2cm}print(str(InputsDecimal[i]), str(Py\_results[i]))\\
\\
